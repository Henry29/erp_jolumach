<template>
  <v-skeleton-loader v-if="firstLoad" :loading="loading" type="table">
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Tipo de Planilla</v-toolbar-title>
...
export default {
  ...
  methods: {
    ...
    deleteItemConfirm() {
      ...
      axios
        .delete("/tipoPlanilla/borrar/" + id)
        .then(function (response) {
          console.log(response);
          if (response.data === 1) {
            me.text = "Se eliminó el tipo de planilla Cod: " + id;
          } else {
            me.text = "No se eliminó el tipo de planilla";
          }
    ...
    save() {
      let me = this;
      let url = "/tipoPlanilla/guardar"; //Ruta que hemos creado para enviar una tarea y guardarla
      ...
      axios
        ...
        .then(function (response) {
          if (response.data[0].estado_registro === true) {
            ...
            me.text = "No se " + textSave + " el tipo de planilla";
          }
          me.dialog = false;
          me.snackbar = true;
          me.initialize();
