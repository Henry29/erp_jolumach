<template>
  <v-skeleton-loader v-if="firstLoad" :loading="loading" type="table">
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Tipo de Zona</v-toolbar-title>
...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Codigo Sunat", value: "codsunat" },
  ...
  methods: {
    ...
    deleteItemConfirm() {
      ...
      axios
        .delete("/tipoZona/borrar/" + id)
        .then(function (response) {
          console.log(response);
          if (response.data === 1) {
            me.text = "Se eliminó el tipo de Zona Cod: " + id;
          } else {
            me.text = "No se eliminó el tipo de Zona";
          }
    ...
    save() {
      let me = this;
      let url = "/tipoZona/guardar"; //Ruta que hemos creado para enviar una tarea y guardarla
      ...
      axios
        ...
        .then(function (response) {
          if (response.data[0].estado_registro === true) {
            ...
            me.text = "No se " + textSave + " el tipo de Zona";
          }
          me.dialog = false;
          me.snackbar = true;
          me.initialize();
