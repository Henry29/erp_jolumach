<template>
    <v-skeleton-loader v-if="firstLoad" :loading="loading" type="table">
        <v-toolbar class="mb-2" color="indigo accent-3" dark flat>
            <v-toolbar-title>Gestión Régimen Laboral</v-toolbar-title>
        </v-toolbar>
        <v-data-table dense :headers="headers" :items="desserts" sort-by="idmaestro" class="elevation-1">
            <template v-slot:top>
                <v-toolbar flat>
                    <v-toolbar-title>Tipo de Regimen Laboral</v-toolbar-title>
    ...
    export default {
        ...
        methods: {
            ...
            deleteItemConfirm() {
                ...
                axios
                    .delete("/regimenLaboral/borrar/" + id)
                    .then(function (response) {
                        console.log(response);
                        if (response.data === 1) {
                            me.text = "Se eliminó el regimen laboral Cod: " + id;
                        } else {
                            me.text = "No se eliminó el regimen laboral";
                        }
            ...
            save() {
                let me = this;
                let url = "/regimenLaboral/guardar"; //Ruta que hemos creado para enviar una tarea y guardarla
                    ...
                    .then(function (response) {
                        if (response.data[0].estado_registro === true) {
                            me.text =
                                "Se " +
                                textSave +
                                " el regimen laboral Cod: " +
                                response.data[0].idmaestro;
                        } else {
                            me.text = "No se " + textSave + " el regimen laboral";
                        }
                        me.dialog = false;
                        me.snackbar = true;
                        me.initialize();
