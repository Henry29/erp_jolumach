<template>
  <v-skeleton-loader v-if="firstLoad" :loading="loading" type="table">
    <v-data-table
      ...
      :headers="headers"
      :items="desserts"
      sort-by="iddenominacion"
      class="elevation-1"
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Tipo de Vía</v-toolbar-title>
                ...
                <v-container>
                  <v-row>
                    ...
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field
                        v-model="editedItem.iddenominacion"
                        label="Id"
                        v-show="false"
                      ></v-text-field>
                    </v-col>
...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        {
          text: "Id",
          value: "iddenominacion",
          align: " d-none",
        },
        { text: "Codigo", value: "codigo" },
        { text: "Abreviatura", value: "denominacion" },
  ...
  methods: {
    ...
    deleteItemConfirm() {
      let me = this;
      let id = me.editedItem.iddenominacion;
      axios
        .delete("/tipoVia/borrar/" + id)
        .then(function (response) {
    ...
    save() {
      let me = this;
      let url = "/tipoVia/guardar"; //Ruta que hemos creado para enviar una tarea y guardarla
        ...
        .then(function (response) {
          if (response.data[0].estado_registro === true) {
            me.text =
              ...
              textSave +
              " el tipo de Vía Cod: " +
              response.data[0].iddenominacion;
          } else {
