<template>
  <v-skeleton-loader v-if="firstLoad" :loading="loading" type="table">
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Tipo de Contrato Laboral</v-toolbar-title>
...
export default {
  ...
  methods: {
    ...
    deleteItemConfirm() {
      ...
      axios
        .delete("/tipoContratoLaboral/borrar/" + id)
        .then(function (response) {
          console.log(response);
          if (response.data === 1) {
            me.text = "Se eliminó el tipo de contrato laboral Cod: " + id;
          } else {
            me.text = "No se eliminó el tipo de contrato laboral";
          }
    ...
    save() {
      let me = this;
      let url = "/tipoContratoLaboral/guardar"; //Ruta que hemos creado para enviar una tarea y guardarla
        ...
        .then(function (response) {
          if (response.data[0].estado_registro === true) {
            me.text =
              "Se " +
              textSave +
              " el tipo de contrato laboral Cod: " +
              response.data[0].idmaestro;
          } else {
            me.text = "No se " + textSave + " el tipo de contrato laboral";
          }
          me.dialog = false;
          me.snackbar = true;
          me.initialize();
