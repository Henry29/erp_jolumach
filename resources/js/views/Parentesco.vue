<template>
    <v-skeleton-loader v-if="firstLoad" :loading="loading" type="table">
        <v-toolbar class="mb-2" color="indigo accent-3" dark flat>
            <v-toolbar-title>Gestión Parentesco</v-toolbar-title>
        </v-toolbar>
        <v-data-table dense :headers="headers" :items="desserts" sort-by="idmaestro" class="elevation-1">
            <template v-slot:top>
                <v-toolbar flat>
                    <!-- <v-toolbar-title>Parentesco</v-toolbar-title> -->
    ...
    export default {
        ...
        methods: {
            ...
            deleteItemConfirm() {
                ...
                axios
                    .delete("/parentesco/borrar/" + id)
                    .then(function (response) {
                        console.log(response);
                        if (response.data === 1) {
                            me.text = "Se eliminó el parentesco Cod: " + id;
                        } else {
                            me.text = "No se eliminó el parentesco";
                        }
            ...
            save() {
                let me = this;
                let url = "/parentesco/guardar"; //Ruta que hemos creado para enviar una tarea y guardarla
                ...
                axios
                    ...
                    .then(function (response) {
                        if (response.data[0].estado_registro === true) {
                            ...
                            me.text = "No se " + textSave + " el parentesco";
                        }
                        me.dialog = false;
                        me.snackbar = true;
                        me.initialize();
