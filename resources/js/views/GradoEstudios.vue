<template>
    <v-skeleton-loader v-if="firstLoad" :loading="loading" type="table">
        <v-toolbar class="mb-2" color="indigo accent-3" dark flat>
            <v-toolbar-title>Gestión Grado de Estudios</v-toolbar-title>
        </v-toolbar>

        <v-data-table dense :headers="headers" :items="desserts" sort-by="idmaestro" class="elevation-1">
            <template v-slot:top>
                <v-toolbar flat>
                    <v-toolbar-title>Grado de Estudios</v-toolbar-title>
                            ...
                            <v-card-text>
                                <v-container>
                                    <v-row>
                                        ...
                                        <v-col cols="12" sm="6" md="6">
                                            <v-text-field v-model="editedItem.abrev" label="Abreviatura"></v-text-field>
                                        </v-col>
                                        ...
                                        <v-col cols="12" sm="6" md="4">
                                            <v-switch v-model="editedItem.estado" label="Estado" color="indigo darken-2"
                                                hide-details></v-switch>
                                        </v-col>
                                        <v-col cols="12" sm="6" md="4">
                                            <v-text-field v-model="editedItem.idmaestro" label="Id" v-show="false">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                </v-container>
    ...
    export default {
        ...
        methods: {
            ...
            deleteItemConfirm() {
                ...
                axios
                    .delete("/gradoEstudios/borrar/" + id)
                    .then(function (response) {
                        console.log(response);
                        if (response.data === 1) {
                            me.text = "Se eliminó el grado de estudio Cod: " + id;
                        } else {
                            me.text = "No se eliminó el grado de estudio";
                        }
            ...
            save() {
                let me = this;
                let url = "/gradoEstudios/guardar"; //Ruta que hemos creado para enviar una tarea y guardarla
                    ...
                    .then(function (response) {
                        if (response.data[0].estado_registro === true) {
                            me.text =
                                "Se " +
                                textSave +
                                " el grado de estudio Cod: " +
                                response.data[0].idmaestro;
                        } else {
                            me.text = "No se " + textSave + " el grado de estudio";
                        }
                        me.dialog = false;
                        me.snackbar = true;
                        me.initialize();
