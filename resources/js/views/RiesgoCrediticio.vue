<template>
  <v-skeleton-loader v-if="firstLoad" :loading="loading" type="table">
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Riesgo Crediticio</v-toolbar-title>
...
export default {
  ...
  methods: {
    ...
    deleteItemConfirm() {
      ...
      axios
        .delete("/riesgoCrediticio/borrar/" + id)
        .then(function (response) {
          console.log(response);
          if (response.data === 1) {
            me.text = "Se eliminó el riesgo crediticio Cod: " + id;
          } else {
            me.text = "No se eliminó el riesgo crediticio";
          }
    ...
    save() {
      let me = this;
      let url = "/riesgoCrediticio/guardar"; //Ruta que hemos creado para enviar una tarea y guardarla
        ...
        .then(function (response) {
          if (response.data[0].estado_registro === true) {
            me.text =
              "Se " +
              textSave +
              " el riesgo crediticio Cod: " +
              response.data[0].idmaestro;
          } else {
            me.text = "No se " + textSave + " el riesgo crediticio";
          }
          me.dialog = false;
          me.snackbar = true;
          me.initialize();
