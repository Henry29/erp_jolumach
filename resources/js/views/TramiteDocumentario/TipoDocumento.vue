<template>
  <v-skeleton-loader v-if="firstLoad" :loading="loading" type="table">
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Tipo de Documento</v-toolbar-title>
...
export default {
  ...
  methods: {       
    ...
    initialize() {
      let me = this;
      let url = "/getTramite_TipoDocumento";
      axios
        .get(url)
    ...
    deleteItemConfirm() {
      ...
      axios
        .delete("/Tramite_TipoDocumento/borrar/" + id)
        .then(function (response) {
          console.log(response);
          if (response.data === 1) {
            me.text = "Se eliminó el tipo de Documento con Cod: " + id;
          } else {
            me.text = "No se eliminó el tipo de Documento";
          }
    ...
    save() {
      let me = this;
      let url = "/Tramite_TipoDocumento/guardar"; //Ruta que hemos creado para enviar una tarea y guardarla
        ...
        .then(function (response) {
          if (response.data[0].estado_registro === true) {
            me.text =
              "Se " +
              textSave +
              " el tipo de Documento con Cod: " +
              response.data[0].identificador_ane;
          } else {
            me.text = "No se " + textSave + " el tipo de Documento";
          }
          me.dialog = false;
          me.snackbar = true;
          me.initialize();
