<template>
  <v-skeleton-loader v-if="firstLoad" :loading="loading" type="table">
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Prioridad</v-toolbar-title>
...
export default {
  ...
  methods: {       
    ...
    deleteItemConfirm() {
      ...
      axios
        .delete("/Tramite_Prioridad/borrar/" + id)
        .then(function (response) {
          console.log(response);
          if (response.data === 1) {
            me.text = "Se eliminó la Prioridad con Cod: " + id;
          } else {
            me.text = "No se eliminó la Prioridad";
          }
    ...
    save() {
      let me = this;
      let url = "/Tramite_Prioridad/guardar"; //Ruta que hemos creado para enviar una tarea y guardarla
        ...
        .then(function (response) {
          if (response.data[0].estado_registro === true) {
            me.text =
              "Se " +
              textSave +
              " la Prioridad con Cod: " +
              response.data[0].identificador_ane;
          } else {
            me.text = "No se " + textSave + " la Prioridad";
          }
          me.dialog = false;
          me.snackbar = true;
          me.initialize();
