<template>
  <v-skeleton-loader v-if="firstLoad" :loading="loading" type="table">
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Calificación</v-toolbar-title>
...
export default {
  ...
  methods: {       
    ...
    initialize() {
      let me = this;
      let url = "/getTramite_Calificacion";
      axios
        .get(url)
    ...
    deleteItemConfirm() {
      ...
      axios
        .delete("/Tramite_Calificacion/borrar/" + id)
        .then(function (response) {
          console.log(response);
          if (response.data === 1) {
            me.text = "Se eliminó la Calificación con Cod: " + id;
          } else {
            me.text = "No se eliminó la Calificación";
          }
    ...
    save() {
      let me = this;
      let url = "/Tramite_Calificacion/guardar"; //Ruta que hemos creado para enviar una tarea y guardarla
      ...
      axios
        ...
        .then(function (response) {
          if (response.data[0].estado_registro === true) {
            ...
            me.text = "No se " + textSave + " la Calificación";
          }
          me.dialog = false;
          me.snackbar = true;
          me.initialize();
