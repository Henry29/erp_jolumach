<template>
  <v-skeleton-loader v-if="firstLoad" :loading="loading" type="table">
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Áreas Coorporativas</v-toolbar-title>
            ...
            <v-card>
              ...
              <v-layout justify-center>
                <v-card-text class="text-center">
                  <v-img src="images/delete_trash.png"></v-img>
                </v-card-text>
              </v-layout>
              <v-spacer></v-spacer>
              <v-layout justify-center>
...
export default {
  ...
  methods: {       
    ...
    initialize() {
      let me = this;
      let url = "/getTramite_AreaCooperativa";
      axios
        .get(url)
    ...
    deleteItemConfirm() {
      ...
      axios
        .delete("/Tramite_areacooperativa/borrar/" + id)
        .then(function (response) {
          console.log(response);
          if (response.data === 1) {
            me.text = "Se eliminó el área Coorporativa con  Cod: " + id;
          } else {
            me.text = "No se eliminó el área Coorporativa";
          }
    ...
    save() {
      let me = this;
      let url = "/Tramite_areacooperativa/guardar"; //Ruta que hemos creado para enviar una tarea y guardarla
        ...
        .then(function (response) {
          if (response.data[0].estado_registro === true) {
            me.text =
              "Se " +
              textSave +
              " el área Coorporativa con Cod: " +
              response.data[0].identificador_ane;
          } else {
            me.text = "No se " + textSave + " el área Coorporativa";
          }
          me.dialog = false;
          me.snackbar = true;
          me.initialize();
